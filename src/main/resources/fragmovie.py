...
zywoo_gameplay = zywoo_gameplay.with_effects([vfx.Freeze(13, 1)])
...
therock_nok = therock_nok.with_start(zywoo_gameplay.start + 13)
...
AUTO_GENERATED_CLIP_0 = AudioFileClip("background.mp3").subclipped(25, 36).with_start(monesy_gameplay.start).with_end(36).with_effects([afx.MultiplyVolume(1,0)])
...
ohmygod = ohmygod.with_effects([vfx.MaskColor(color=(0, 255, 8), threshold=100, stiffness=5)])
...
shocked = ImageClip("shocked.png").with_start(monesy_gameplay.start + 5).with_duration(monesy_gameplay.start + monesy_gameplay.end - 5)
...
AUTO_GENERATED_CLIP_1 = AudioFileClip("formula.mp3").subclipped(0, hrek.end).with_start(hrek.start).with_end(hrek.end).with_effects([afx.MultiplyVolume(1,0)])
